"""Supply chain scanner API routes."""
...
router = APIRouter(
    prefix="/supply-chain-scanner",
    tags=["Supply Chain Scanner"],
)
_runner: Any = None
...
def _get_runner() -> Any:
    if _runner is None:
        raise HTTPException(
            503,
            "supply_chain_scanner unavailable",
        )
    return _runner
...
async def run_agent(
    ...
    try:
        ...
        logger.exception("supply_chain_scanner.run.error")
