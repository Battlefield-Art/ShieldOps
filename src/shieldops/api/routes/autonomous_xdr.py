...
router = APIRouter(prefix="/autonomous-xdr", tags=["Autonomous XDR"])
_runner: Any = None
...
def _get_runner() -> Any:
    if _runner is None:
        raise HTTPException(503, "autonomous_xdr service unavailable")
    return _runner
...
async def run_agent(
    ...
    runner = _get_runner()
    try:
        result = await runner.detect(tenant_id=body.tenant_id)
        return {"status": "completed", "result": result}
    except Exception as e:
        logger.exception("autonomous_xdr.run.error")
