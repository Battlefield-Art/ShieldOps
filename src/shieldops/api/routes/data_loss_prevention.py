"""Data Loss Prevention API routes."""
...
router = APIRouter(prefix="/data-loss-prevention", tags=["Data Loss Prevention"])
...
def _get_runner() -> Any:
    if _runner is None:
        raise HTTPException(503, "data_loss_prevention service unavailable")
    return _runner
...
async def run_agent(
    ...
    runner = _get_runner()
    try:
        result = await runner.protect(tenant_id=body.tenant_id)
        return {"status": "completed", "result": result}
    except Exception as e:
        logger.exception("data_loss_prevention.run.error")
