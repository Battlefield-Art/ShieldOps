...
router = APIRouter(prefix="/ai-soc-assistant", tags=["AI SOC Assistant"])
_runner: Any = None
...
def _get_runner() -> Any:
    if _runner is None:
        raise HTTPException(503, "ai_soc_assistant service unavailable")
    return _runner
...
async def run_agent(
    ...
    runner = _get_runner()
    try:
        result = await runner.ask(tenant_id=body.tenant_id)
        return {"status": "completed", "result": result}
    except Exception as e:
        logger.exception("ai_soc_assistant.run.error")
