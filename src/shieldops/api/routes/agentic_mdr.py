...
router = APIRouter(prefix="/agentic-mdr", tags=["Agentic MDR"])
...
def _get_runner() -> Any:
    if _runner is None:
        raise HTTPException(503, "agentic_mdr service unavailable")
    return _runner
...
async def run_agent(
    ...
    runner = _get_runner()
    try:
        result = await runner.respond(tenant_id=body.tenant_id)
        return {"status": "completed", "result": result}
    except Exception as e:
        logger.exception("agentic_mdr.run.error")
