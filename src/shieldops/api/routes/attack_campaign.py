...
router = APIRouter(prefix="/attack-campaign", tags=["Attack Campaign"])
...
def _get_runner() -> Any:
    if _runner is None:
        raise HTTPException(503, "attack_campaign service unavailable")
    return _runner
...
async def run_agent(
    ...
    try:
        ...
        logger.exception("attack_campaign.run.error")
