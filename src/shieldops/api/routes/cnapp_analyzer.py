...
router = APIRouter(prefix="/cnapp-analyzer", tags=["CNAPP Analyzer"])
_runner: Any = None
...
def _get_runner() -> Any:
    if _runner is None:
        raise HTTPException(503, "cnapp_analyzer service unavailable")
    return _runner
...
async def run_agent(
    ...
    try:
        ...
        logger.exception("cnapp_analyzer.run.error")
