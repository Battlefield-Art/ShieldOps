...
def _get_runner() -> Any:
    if _runner is None:
        raise HTTPException(503, "Policy Engine service unavailable")
    return _runner
...
async def run_agent(
    ...
    runner = _get_runner()
    try:
        result = await runner.evaluate(tenant_id=body.tenant_id)
        return {"status": "completed", "result": result}
    except Exception as e:
        logger.exception("policy_engine.run.error")
