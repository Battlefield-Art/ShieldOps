...
router = APIRouter(prefix="/cyber-recovery", tags=["Cyber Recovery"])
_runner: Any = None
...
def _get_runner() -> Any:
    if _runner is None:
        raise HTTPException(503, "cyber_recovery service unavailable")
    return _runner
...
async def run_agent(
    ...
    runner = _get_runner()
    try:
        result = await runner.recover(tenant_id=body.tenant_id)
        return {"status": "completed", "result": result}
    except Exception as e:
        logger.exception("cyber_recovery.run.error")
