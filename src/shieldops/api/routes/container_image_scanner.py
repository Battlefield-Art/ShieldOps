...
router = APIRouter(
    prefix="/container-image-scanner",
...
def _get_runner() -> Any:
    if _runner is None:
        raise HTTPException(
            503,
            "container_image_scanner service unavailable",
        )
    return _runner
...
async def health() -> dict[str, Any]:
    return {
        "service": "container_image_scanner",
...
async def run_agent(
    ...
    try:
        ...
        logger.exception("container_image_scanner.run.error")
