...
router = APIRouter(prefix="/cloud-risk-ranker", tags=["Cloud Risk Ranker"])
...
def _get_runner() -> Any:
    if _runner is None:
        raise HTTPException(503, "cloud_risk_ranker service unavailable")
    return _runner
...
async def run_agent(
    ...
    runner = _get_runner()
    try:
        result = await runner.rank(tenant_id=body.tenant_id)
        return {"status": "completed", "result": result}
    except Exception as e:
        logger.exception("cloud_risk_ranker.run.error")
