...
router = APIRouter(prefix="/intelligent-soar", tags=["Intelligent SOAR"])
_runner: Any = None
...
def _get_runner() -> Any:
    if _runner is None:
        raise HTTPException(503, "intelligent_soar service unavailable")
    return _runner
...
async def run_agent(
    ...
    runner = _get_runner()
    try:
        result = await runner.orchestrate(tenant_id=body.tenant_id)
        return {"status": "completed", "result": result}
    except Exception as e:
        logger.exception("intelligent_soar.run.error")
