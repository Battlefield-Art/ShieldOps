...
router = APIRouter(prefix="/breakout-defender", tags=["Breakout Defender"])
...
def _get_runner() -> Any:
    if _runner is None:
        raise HTTPException(503, "breakout_defender service unavailable")
    return _runner
...
async def run_agent(
    ...
    runner = _get_runner()
    try:
        result = await runner.defend(tenant_id=body.tenant_id)
        return {"status": "completed", "result": result}
    except Exception as e:
        logger.exception("breakout_defender.run.error")
